...
def main(root_dir, seq, raw_file_template, seg_file_template, tra_file_template=None, force=False, FOV=80,
    ...
    valid_seg = None if not force_yes_to_all else 'yes_to_all'
    while True:
        if os.path.basename(raw_file_template.format(t)) in all_image_files:
            ...
            tra_fname = tra_fname if tra_fname and os.path.basename(tra_fname) in all_tra_files else None
            ...
            if os.path.basename(seg_file_template.format(t)) in all_seg_files:
                ...
                if valid_seg == 'yes_to_all':
                    ...
                    plt.pause(0.1)
                    valid_seg = input(
                        'Frame {}: Are all cells in the frame annotated [Y/n/yes to all]? '.format(t)).lower()
                ...
                row = (im_fname, seg_fname, tra_fname, valid_seg in ['', 'y', 'yes', 'yes_to_all'])
            else:
...
def get_default_run():
    ...
    seg_file_template = os.path.join('{}_ST'.format(seq), 'SEG', 'man_seg{:03d}.tif')
...
if __name__ == '__main__':
    ...
    for s in range(1, 3):
        ...
        # seq_metadata[filelist] will be a list of tuples holding (raw_fname, seg_fname, tra_filename, is_seg_val)
        # the format for 'BF-C2DL-HSC-ST','BF-C2DL-MuSC-ST' is that of t{:04d}.tif where for the rest it is t{:03d}.tif
        raw_file_template_ = os.path.join(seq_, 't{:03d}.tif')
        seg_file_template_ = os.path.join('{}_GT'.format(seq_), 'SEG', 'man_seg{:03d}.tif')
        # seg_file_template_ = os.path.join('{}_ST'.format(seq_), 'SEG', 'man_seg{:03d}.tif')
        tra_file_template_ = os.path.join('{}_GT'.format(seq_), 'TRA',
                                         'man_track{:03d}.tif')  # Optional
        ...
        main(root_dir_, seq_, raw_file_template_, seg_file_template_, tra_file_template_, input_args.force,
             force_yes_to_all=force_yes_to_all)
