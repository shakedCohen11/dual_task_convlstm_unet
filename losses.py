...
class WeightedCELoss(object):
    def __init__(self, channel_axis, class_weights, num_of_classes=3):
    ...
    def __call__(self, gt_sequence, output_sequence):
        ...
        onehot_gt = tf.one_hot(tf.cast(gt_sequence, tf.int32), self.num_of_classes)
...
class WeightedCELoss3D(object):
    ...
    def __call__(self, gt_sequence, output_sequence):
        ...
        gt_sequence = tf.cast(gt_sequence, tf.int32)
        if self.channel_axis == 2:
            output_sequence = tf.transpose(output_sequence, (0, 1, 3, 4, 5, 2))
...
def seg_measure(channel_axis, three_d=False, foreground_class_index=1):
    ...
    # this function gets un labeled seg images and labels the connected components then
    # measures the IOU of each label, only if it is greater then 0.5 it gets a value,
    # and the function returns the mean IOU
    def seg_numpy(gt, seg):
        # get lables
        gt_labled, _ = connencted_components(gt.numpy())
        seg_labled, _ = connencted_components(seg.numpy())

        # calculte the IOU for each lable:
        all_iou = []
            ...
            for gt, seg in zip(gt1, seg1):
                for this_label in np.unique(gt):
                    ...
                    for s in np.unique(overlaping_inds):
                        ...
                        if overlap > 0.5: # add else for score of 0
...
def seg_measure_compare(seg1, seg2):
    # set iou array for each segmented cell
    all_iou = []
    for this_label in np.unique(seg1):
        # for each cell segment in seg 1 claulate the labeled area
        # and find the overlappping area in seg 2
        if this_label == 0:
            continue
        all_iou.append(0.)
        bw = seg1 == this_label
        l_area = np.sum(bw).astype(np.float32)
        overlaping_inds = seg2[bw]
        for s in np.unique(overlaping_inds):
            # for each cell segment in seg 2 that over lapps the
            # current label calculate the intersection area
            if s == 0:
                continue
            intersection = np.sum(overlaping_inds == s).astype(np.float32)
            # calculate the over lapp of the label and seg area
            overlap = intersection / l_area
            if overlap > 0.5:
                # calculate iou if over lapp is over 0.5 and add to the iou array
                s_area = np.sum(seg2 == s).astype(np.float32)
                iou = intersection / (l_area + s_area - intersection)
                all_iou[-1] = iou
    if not len(all_iou):
        return np.nan
