...
class CodeBlock2D(k.Model):

    def __init__(self,  conv_kernels: List[tuple], lstm_kernels: List[tuple], stride=1, data_format='NCHW'):
        super(CodeBlock2D, self).__init__()
        data_format_keras = 'channels_first' if data_format[1] == 'C' else 'channels_last'
        channel_axis = 1 if data_format[1] == 'C' else -1
        self.ConvLSTM = []
        self.Conv = []
...
class ULSTMnet2D(k.Model):
    def __init__(self, net_params=DEFAULT_NET_DOWN_PARAMS, data_format='NCHW', pad_image=True, add_binary_output=False,
        ...
        if add_binary_output:
            if sigmoid_output:
                ...
                conv_filters[-1] = (1, 2)
                self.BinaryOutputLogits = UpBlock2D(conv_filters, up_factor, data_format, return_logits=True)
                self.BinaryOutputSoftmax = k.layers.Softmax(self.channel_axis + 1)
            self.binary_depth = conv_filters[-1][1]
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        if self.channel_axis == 1:
            ...
            if self.add_binary_output:
                crops_binary = [[0, input_shape[0]], [0, input_shape[1]], [0, self.binary_depth],
                                [pad_y[0], pad_y[0] + input_shape[3]], [pad_x[0], pad_x[0] + input_shape[4]]]
        else:
            ...
            if self.add_binary_output:
                crops_binary = [[0, input_shape[0]], [0, input_shape[1]], [pad_y[0], input_shape[2] + pad_y[0]],
                                [pad_x[0], input_shape[3] + pad_x[0]], [0, self.binary_depth]]
        ...
        for up_layer, skip_input in zip(self.UpLayers, skip_inputs):
            if self.add_binary_output and up_layer is self.UpLayers[-1]:
                ...
                if self.sigmoid_output:
                    ...
                    binary_output_softmax = self.BinaryOutputSoftmax(binary_output_logits)
        ...
        if self.add_binary_output:
            if self.sigmoid_output:
                ...
                return logits_output, softmax_output, binary_output_softmax, binary_output_logits
    ...
    def unit_test(cls):
        ...
        model = cls(net_params, data_format, pad_image, add_binary_outputs, sigmoid_output)
        for i in range(4):
            input_sequence = np.random.randn(batch_size, unroll_len, h, w, d).astype(np.float32)
...
class ULSTMnet2D2Decoder(k.Model):
    def __init__(self, net_params=DEFAULT_NET_DOWN_PARAMS, data_format='NCHW', pad_image=True, add_binary_output=False,
                 sigmoid_output=True, add_code_layer=False):
        super(ULSTMnet2D2Decoder, self).__init__()
        self.data_format_keras = 'channels_first' if data_format[1] == 'C' else 'channels_last'
        self.channel_axis = 1 if data_format[1] == 'C' else -1
        self.DownLayers = []
        self.UpLayersSeg = []
        ...
        self.pad_image = pad_image
        self.add_binary_output = add_binary_output
        self.add_code_layer = add_code_layer
        self.CodeLayers = []
        self.sigmoid_output = sigmoid_output
        ...
        for layer_ind, (conv_filters, lstm_filters) in enumerate(zip(net_params['down_conv_kernels'],
            ...
            self.total_stride *= self.DownLayers[-1].total_stride
        if add_code_layer:
            self.CodeLayers.append(CodeBlock2D(net_params['down_conv_kernels'][-1], net_params['lstm_kernels'][-1], 1,
                                               data_format))
        for layer_ind, conv_filters in enumerate(net_params['up_conv_kernels']):
            up_factor = 2 if layer_ind > 0 else 1
            self.UpLayersSeg.append(UpBlock2D(conv_filters, up_factor, data_format,
                                              return_logits=layer_ind + 1 == len(net_params['up_conv_kernels'])))
            self.last_depth = conv_filters[-1][1]
        ...
        if add_binary_output:
            if add_code_layer:
                self.CodeLayers.append(
                    CodeBlock2D(net_params['down_conv_kernels'][-1], net_params['lstm_kernels'][-1], 1, data_format))
            for layer_ind, conv_filters in enumerate(net_params['up_conv_kernels']):
                up_factor = 2 if layer_ind > 0 else 1
                if conv_filters is net_params['up_conv_kernels'][-1]:
                    if sigmoid_output:
                        conv_filters[-1] = (1, 1)
                    else:
                        conv_filters[-1] = (1, 2)
                self.UpLayersTra.append(UpBlock2D(conv_filters, up_factor, data_format,
                                                  return_logits=layer_ind + 1 == len(net_params['up_conv_kernels'])))
                # self.last_depth = conv_filters[-1][1]
            # up_factor=2
            # conv_filters = net_params['up_conv_kernels'][-1].copy()
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        for down_layer in self.DownLayers:
            skip_inputs.append(out_skip)
            out_down, out_skip = down_layer(out_down, training=training, mask=mask)
        if self.add_code_layer:
            _, out_skip_seg = self.CodeLayers[0](out_down)
            up_input_seg = out_skip_seg
        else:
            up_input_seg = out_skip
        skip_inputs.reverse()

        assert len(skip_inputs) == len(self.UpLayersSeg)
        for up_layer_seg, skip_input in zip(self.UpLayersSeg, skip_inputs):
            up_input_seg = up_layer_seg((up_input_seg, skip_input), training=training, mask=mask)

        logits_output_shape = up_input_seg.shape
        logits_output = tf.reshape(up_input_seg, [input_shape[0], input_shape[1], logits_output_shape[1],
                                              logits_output_shape[2], logits_output_shape[3]])
        ...
        if self.add_binary_output:
            if self.add_code_layer:
                _, out_skip_tra = self.CodeLayers[0](out_down)
                up_input_tra = out_skip_tra
            else:
                up_input_tra = out_skip
            assert len(skip_inputs) == len(self.UpLayersTra)
            for up_layer_tra, skip_input in zip(self.UpLayersTra, skip_inputs):
                up_input_tra = up_layer_tra((up_input_tra, skip_input), training=training, mask=mask)

            # binary_output_logits = self.BinaryOutputLogits((up_input, skip_input), training=training, mask=mask)

            binary_output_shape = up_input_tra.shape
            binary_output_logits = tf.reshape(up_input_tra,
                                              [input_shape[0], input_shape[1], binary_output_shape[1],
                                               binary_output_shape[2], binary_output_shape[3]])
            binary_output_logits = binary_output_logits[crops_binary[0][0]:crops_binary[0][1],
                                   crops_binary[1][0]:crops_binary[1][1], crops_binary[2][0]:crops_binary[2][1],
...
if __name__ == "__main__":
    ...
    # ULSTMnet2D2Decoder.unit_test()
    CodeBlock2D.unit_test()